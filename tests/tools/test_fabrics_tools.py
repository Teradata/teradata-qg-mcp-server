...
async def test_qg_get_fabrics_with_flatten(mcp_client: Client):
    ...
    result = await mcp_client.call_tool(
        "qg_get_fabrics", arguments={"flatten": True}
    )
...
async def test_qg_get_fabric_by_id_with_extra_info(mcp_client: Client, test_fabric):
    ...
    result = await mcp_client.call_tool(
        "qg_get_fabric_by_id", 
        arguments={"id": fabric_id, "extra_info": True}
    )
...
@pytest.mark.integration
async def test_qg_create_fabric_different_auth_key_sizes(mcp_client: Client, qg_manager):
